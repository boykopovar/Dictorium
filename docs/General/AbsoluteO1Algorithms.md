...
Пусть $b = |\text{bucket}|$, $s = \mathtt{tableSize} \approx b^2$.
...
$$\Rightarrow W(b) = O(\mathtt{MAX\_ATTEMPTS} \cdot b^2) = \Theta(b^2), \quad A(b) = \Theta(b^2)$$
...
Обозначим $n = \mathtt{size}$, $b_k$ — размер $k$-й корзины, $s_k = \mathtt{innerSize}_k \approx b_k^2$.
...
$$W(n) = O(n \cdot \mathtt{MAX\_ATTEMPTS}^2) = O(n) \text{ (с высокой вероятностью)}$$
